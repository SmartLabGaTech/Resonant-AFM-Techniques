...
def getAcqusitionSpectrum(allData,freqApplied,lowerFreq,upperFreq,samplingFreq,acqNum,chirpMap=None):

	#Get the names of the chirps
	chirps=allData.index.get_level_values(0)
	chirpNames=chirps.unique()
	numChirps=len(chirpNames)

	#Extract Acquisition
	acqData=allData.xs(acqNum,axis=1)

	fres=np.ceil(1/(freqApplied.size*(1/samplingFreq)))
	fftLen=np.round(((upperFreq-lowerFreq)/fres)-1)

	outData=np.empty([numChirps,int(fftLen)], dtype=np.complex)

	chirpIterator=np.arange(1,numChirps+1)

	#For each chirp get the FFT
	for i in chirpIterator:

		#Extract chirp
		chirpData=acqData.xs('Chirp'+str(i))
		
		#Extract chirp 
		if chirpMap is None:
			harmonic=1
		else:
			info=chirpMap.loc[i-1,:]
		...
		if harmonic == 1:
			...
			off=freqRange/2
			center=(upperFreq+lowerFreq)/2
			newCenter=center*harmonic
			low=newCenter-off
			upper=newCenter+off
		
		#Get the spectrum data (i.e. fft)
		freq,response=getSpecWaveform(chirpData,freqApplied,low,upper,samplingFreq)
		
		#response=savitzky_golay(response, 53, 3)
		
		outData[i-1,:]=response
...
def generateChirpMap(waveformSpecPath):
	...
	numChirps=np.sum(waveSpec.loc[:,cols[0]].values)
...
def fftAnalysis(acqNum,file,samplingFreq, chirpDuration, lowerFreq, upperFreq, freqApplied, tdmsFile, chirpMap=None):
	...
	spec=getAcqusitionSpectrum(chop,freqApplied,lowerFreq,upperFreq,samplingFreq,acqNum,chirpMap)
